...
export default function Carousel() {
    ...
    >
      ...
      {slides.map((slide) => (
        <SwiperSlide key={slide.id}>
          <div className="relative w-full overflow-hidden h-[700px] md:h-screen ">
              <Image
    src={slide.image}
    alt={slide.title}
    fill
  ...
  />
          <div className="flex flex-col gap-4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
           <h2 className=" text-white text-center text-[20px] md:text-3xl megatitle font-medium uppercase">
              {slide.title}
            </h2>
            <p className=" text-white text-center text-[14px] md:text-[16px] font-fam-parag leading-4">
              {slide.desc}
            </p>
